...
// ── Timeline ─────────────────────────────────────────────────────────────────
function TimelineBar({ phases, nameA, nameB }: {
  ...
  function label(type: string) {
    return SEGMENT_LABELS[type]
      ?.replace('A spricht', `${nameA} spricht`)
      .replace('Abschluss A', `Abschluss ${nameA}`)
      .replace('A', nameA)
      .replace('B spricht', `${nameB} spricht`)
      .replace('Abschluss B', `Abschluss ${nameB}`)
      ?? type
  }
...
export default function SetupPage() {
  ...
  // Build mode with customized transition durations
  const mode = useMemo(() => {
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white px-4 py-6 max-w-lg mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center gap-2 mt-3 text-slate-400 text-sm">
          ...
          <span>Gesamtdauer: <strong className="text-slate-600">{formatDurationShort(totalSec)}</strong></span>
        </div>
      </div>

      {/* ── Partner-Namen ── */}
      <section className="mb-8">
        <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Partner-Namen</h2>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs text-slate-400 mb-1">Person A</label>
            <input
              type="text"
              value={localNameA}
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Person B</label>
            <input
              type="text"
              value={localNameB}
        ...
        <p className="text-xs text-slate-400 mt-2">
          Die Namen erscheinen auf dem Timer-Bildschirm. Nur lokal gespeichert.
        </p>
      </section>
      ...
      {mode.phases.some(p => p.type === 'transition') && (
        <section className="mb-8">
          ...
          <p className="text-xs text-slate-400 mb-3">
            Stille Phase nach jeder Sprechzeit. Verarbeiten, bevor die nächste Person spricht.
          </p>
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <span className="text-xs text-amber-600 font-medium">
              {isQuick ? 'Minimal' : localTrans >= 120 ? 'Großzügig' : 'Standard'}
            </span>
          </div>

          {localTrans === 0 && (
            <p className="text-xs text-amber-500 mt-1">
              Kein Übergang – die nächste Phase startet sofort.
            </p>
          )}
        </section>
      )}
      ...
      <section className="mb-8">
        ...
        <TimelineBar phases={mode.phases} nameA={localNameA || 'A'} nameB={localNameB || 'B'} />
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-500">
          <div>
            Phasen: <strong className="text-slate-700">{mode.phases.length}</strong>
          </div>
          <div>
            Gesamtdauer: <strong className="text-slate-700">{formatDurationShort(totalSec)}</strong>
          </div>
          {mode.phases.some(p => p.type === 'slotA') && (
            <div>
              Runden: <strong className="text-slate-700">
          ...
          {mode.phases.some(p => p.type === 'cooldown' && p.duration > 0) && (
            <div>
              Nachgesprächs-Pause: <strong className="text-slate-700">
      ...
      <p className="text-xs text-slate-400 text-center mt-4">
        Der Gong startet sofort. Bitte vorher vorbereiten.
      </p>
    </main>
  )
}
