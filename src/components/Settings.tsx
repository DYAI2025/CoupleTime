...
import { BackgroundConfig } from '../domain/BackgroundConfig'
import { ParticipantConfig } from '../domain/ParticipantConfig'
import { backgroundPersistenceService } from '../services/BackgroundPersistenceService'

/**
 * Enhanced Settings panel with all new features
 */
export function EnhancedSettingsButton() {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && <EnhancedSettingsPanel onClose={() => setIsOpen(false)} />}
      </AnimatePresence>
    </>
  )
}
...
/**
 * Enhanced Settings panel modal
 */
function EnhancedSettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  const [backgroundConfig, setBackgroundConfig] = useState(backgroundPersistenceService.loadConfig());
  ...
  useEffect(() => {
    backgroundPersistenceService.saveConfig(backgroundConfig);
    
    // Apply background color to body
    document.body.style.backgroundColor = backgroundConfig.primaryColor;
  }, [backgroundConfig]);
  ...
  const handleParticipantChange = (config: ParticipantConfig) => {
    session.updateParticipantConfig(config);
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-4 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            <h3 className="font-medium text-gray-800 dark:text-white mb-2">
              {t('common.background', 'Background Settings')}
            </h3>
            <BackgroundSettings config={backgroundConfig} onSave={handleBackgroundChange} />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-medium text-gray-800 dark:text-white">
                {t('settings.participant.title', 'Participant Names')}
              </h3>
              <button
                onClick={() => setShowSessionSetup(true)}
                className="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
              >
                {t('settings.participant.edit', 'Edit')}
              </button>
            </div>
            <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                {t('settings.participant.current', 'Current names:')} <span className="font-medium">{session.participantConfig.nameA}</span> & <span className="font-medium">{session.participantConfig.nameB}</span>
              </div>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  aria-label={t('settings.autoRotate', 'Auto-rotate Interval')}
                />
      ...
      {/* Session Setup Modal - conditionally rendered */}
      {showSessionSetup && (
        <SessionSetup 
          config={session.participantConfig} 
...
export default EnhancedSettingsButton